...
if (isset($_FILES['rocket_money_file']) && $_FILES['rocket_money_file']['error'] === UPLOAD_ERR_OK) {
    ...
    if (!in_array($fileExtension, $allowedExtensions)) {
        header("Location: audit_expenses_v2.php?error=" . urlencode("Invalid file type. Please upload a CSV file."));
        exit;
    }
    
    // Create destination directory if it doesn't exist
    $destinationDir = '../../data/audit_v2';
    ...
    if (move_uploaded_file($tempPath, $destinationPath)) {
        header("Location: audit_expenses_v2.php?Message=" . urlencode("File uploaded successfully: " . $newFileName) . "&file=" . urlencode($newFileName));
        exit;
    } else {
        header("Location: audit_expenses_v2.php?error=" . urlencode("Failed to save uploaded file."));
        exit;
    }
} else {
    ...
    header("Location: audit_expenses_v2.php?error=" . urlencode($errorMessage));
    exit;
}