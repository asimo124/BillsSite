//
//  AssociationCodesUdderScoreTeatScore.swift
//  IOSApp3
//
...
class AssociationCodesUdderScoreTeatScoreData {
    ...
    func setText(text: String?) {
        self.text = text ?? ""
        self.text_str = String(self.text)
    }
    func getText() -> String {
        return self.text
    }
    ...
    func getTextStr() -> String {
        return text_str == "unset" ? "" : text_str
    };
    func getTextStrPG() -> String {
        return text_str == "unset" ? "" : text_str
    };
    ...
    func loadByID(_id: String) -> [String: String] {
        let qb = QueryBuilder()
        qb.selectFrom("codes_udder_teat_score")
            .Where("_id = ?")
        ...
        if (error != nil) {
            ...
            if resultset.count > 0 {
                ...
                self.setText((row["text"]?.asString()));
                
                
                fields["_id"] = self.getIdStr()
                fields["code"] = self.getCodeStr()
                fields["text"] = self.getTextStr()
    ...
    func getFieldsPG() -> [String: String] {
        ...
        fields["text"] = self.getTextStrPG()
        return fields    }
    
    func addNew() {
        
        var statement = "INSERT INTO codes_udder_teat_score ("
        var i = 0
        let obj = self
        let reflected = Mirror(reflecting: obj)
        for child in reflected.children {
    ...
    func update(forID: Int) {
        ...
        var statement = "UPDATE codes_udder_teat_score "
        var i = 0
        let obj = self
        let reflected = Mirror(reflecting: obj)
        for child in reflected.children {
    ...
    func delete(forID: Int) {
        let query = "DELETE FROM codes_udder_teat_score WHERE _id = " + String(forID)
        let dbn = Database()
        dbn.query(query)
    }
    ...
    func deletePG(forID: Int) {
        ...
        let statement = "DELETE FROM codes_udder_teat_score WHERE _id = " + String(forID)
        let dbn = Database()
        dbn.query("INSERT INTO " + SyncableQueries.TABLE_NAME
            + " (" + SyncableQueries.SQL_QUERY + ") "
            + " VALUES "
    ...
    class func getLastID() -> String {
        var _id: String = ""
        let query = "SELECT _id from codes_udder_teat_score ORDER BY _id DESC LIMIT 1 "
