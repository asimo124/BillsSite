...
class AssociationCodesStatusData {
    ...
    func setStatus(status: String?) {
        self.status = status ?? ""
        self.status_str = String(self.status)
    }
    ...
    func loadByID(_id: String) -> [String: String] {
        ...
        if (error != nil) {
            ...
            if resultset.count > 0 {
                ...
                self.setStatus((row["status"]?.asString()));
    ...
    func addNew() {
        ...
        var statement = "INSERT INTO codes_status ("
        var i = 0
        let obj = self
        let reflected = Mirror(reflecting: obj)
        for child in reflected.children {
    ...
    func update(forID: Int) {
        ...
        var statement = "UPDATE codes_status "
        var i = 0
        let obj = self
        let reflected = Mirror(reflecting: obj)
        for child in reflected.children {
    ...
    func delete(forID: Int) {
        let query = "DELETE FROM codes_status WHERE _id = " + String(forID)
        let dbn = Database()
        dbn.query(query)
    }
    ...
    func deletePG(forID: Int) {
        ...
        let statement = "DELETE FROM codes_status WHERE _id = " + String(forID)
        let dbn = Database()
        dbn.query("INSERT INTO " + SyncableQueries.TABLE_NAME
            + " (" + SyncableQueries.SQL_QUERY + ") "
            + " VALUES "
    ...
    class func getLastID() -> String {
        let _id: String = ""
        let query = "SELECT _id from codes_status ORDER BY _id DESC LIMIT 1 "
