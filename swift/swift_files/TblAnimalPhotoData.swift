...
class TblAnimalPhotoData {
	...
	var hide_from_rotation: Int = -1
    var hide_from_rotation_str: String = "unset"
	...
	func setPhoto(photo: String?) {
        self.photo = photo ?? ""
        self.photo_str = String(self.photo)
    }
	...
	func getPhotoStr() -> String {
        return photo_str == "unset" ? "" : photo_str
    };
	func getPhotoStrPG() -> String {
        return photo_str == "unset" ? "" : photo_str
    };
	...
	func setHideFromRotation(hide_from_rotation: Int?) {
        self.hide_from_rotation = hide_from_rotation ?? 0
        self.hide_from_rotation_str = ((self.hide_from_rotation) == 1) ? "t" : "f"
    }
    func getHideFromRotation() -> Int {
        return self.hide_from_rotation
    }
    func getHideFromRotationType() -> String {
        return "Int"
    };
	func getHideFromRotationStr() -> String {
        return String(hide_from_rotation)
    };
	func getHideFromRotationStrPG() -> String {
        return hide_from_rotation_str
    };
	...
	func getFieldString(field: String) -> String {
        switch field { 
			...
			case "hide_from_rotation":
			return self.getHideFromRotationStr()
			case "comment":
    ...
    func loadByID(animal_registration: String) -> [String: String] {
        let qb = QueryBuilder()
        qb.selectFrom("tbl_animal_photo")
        ...
        if (error != nil) {
            ...
            if resultset.count > 0 {
				...
				self.setPhoto((row["photo"]?.asString()));
				...
				fields["hide_from_rotation"] = self.getHideFromRotationStr()
    ...
    func getFieldsPG() -> [String: String] {
		...
		fields["photo"] = self.getPhotoStrPG()
		...
		fields["hide_from_rotation"] = self.getHideFromRotationStrPG()
		fields["comment"] = self.getCommentStrPG()
		return fields    }

    func addNew() {

        var statement = "INSERT INTO tbl_animal_photo ("
        var i = 0
        let obj = self
        let reflected = Mirror(reflecting: obj)
        for child in reflected.children {
    ...
    func update(forID: String) {
        ...
        var statement = "UPDATE tbl_animal_photo "
        var i = 0
        let obj = self
        let reflected = Mirror(reflecting: obj)
        for child in reflected.children {
    ...
    func delete(forID: Int) {
        let query = "DELETE FROM tbl_animal_photo WHERE animal_registration = " + String(forID)
        let dbn = DatabaseOld()
        dbn.query(query)
    }
    ...
    func deletePG(forID: String) {
        ...
        let statement = "DELETE FROM tbl_animal_photo WHERE animal_registration = " + String(forID)
        let dbn = DatabaseOld()
        dbn.query("INSERT INTO " + SyncableQueries.TABLE_NAME
            + " (" + SyncableQueries.SQL_QUERY + ") "
            + " VALUES "
    ...
    class func getLastanimalRegistration() -> String {
        ...
        let query = "SELECT animal_registration from tbl_animal_photo ORDER BY _id DESC LIMIT 1 "
