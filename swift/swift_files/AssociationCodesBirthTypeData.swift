//
//  AssociationCodesBirthType.swift
//  IOSApp3
//
...
class AssociationCodesBirthTypeData {
    ...
    func loadByID(_id: String) -> [String: String] {
        let qb = QueryBuilder()
        qb.selectFrom("codes_birth_type")
            .Where("_id = ?")
    ...
    func addNew() {
        ...
        var statement = "INSERT INTO codes_birth_type ("
        var i = 0
        let obj = self
        let reflected = Mirror(reflecting: obj)
        for child in reflected.children {
    ...
    func update(forID: Int) {
        ...
        var statement = "UPDATE codes_birth_type "
        var i = 0
        let obj = self
        let reflected = Mirror(reflecting: obj)
        for child in reflected.children {
    ...
    func delete(forID: Int) {
        let query = "DELETE FROM codes_birth_type WHERE _id = " + String(forID)
        let dbn = Database()
        dbn.query(query)
    }
    ...
    func deletePG(forID: Int) {
        ...
        let statement = "DELETE FROM codes_birth_type WHERE _id = " + String(forID)
        let dbn = Database()
        dbn.query("INSERT INTO " + SyncableQueries.TABLE_NAME
            + " (" + SyncableQueries.SQL_QUERY + ") "
            + " VALUES "
    ...
    class func getLastID() -> String {
        var _id: String = ""
        let query = "SELECT _id from codes_birth_type ORDER BY _id DESC LIMIT 1 "
