//
//  AssociationCodesBreedCode.swift
//  IOSApp3
//
...
class AssociationCodesBreedCodeData {
    ...
    func loadByID(_id: String) -> [String: String] {
        let qb = QueryBuilder()
        qb.selectFrom("codes_breed_code")
            .Where("_id = ?")
    ...
    func addNew() {
        ...
        var statement = "INSERT INTO codes_breed_code ("
        var i = 0
        let obj = self
        let reflected = Mirror(reflecting: obj)
        for child in reflected.children {
    ...
    func update(forID: Int) {
        ...
        var statement = "UPDATE codes_breed_code "
        var i = 0
        let obj = self
        let reflected = Mirror(reflecting: obj)
        for child in reflected.children {
    ...
    func delete(forID: Int) {
        let query = "DELETE FROM codes_breed_code WHERE _id = " + String(forID)
        let dbn = Database()
        dbn.query(query)
    }
    ...
    func deletePG(forID: Int) {
        ...
        let statement = "DELETE FROM codes_breed_code WHERE _id = " + String(forID)
        let dbn = Database()
        dbn.query("INSERT INTO " + SyncableQueries.TABLE_NAME
            + " (" + SyncableQueries.SQL_QUERY + ") "
            + " VALUES "
    ...
    class func getLastID() -> String {
        var _id: String = ""
        let query = "SELECT _id from codes_breed_code ORDER BY _id DESC LIMIT 1 "
