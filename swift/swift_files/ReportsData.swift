...
class ReportsData {
    ...
    func find() -> [ReportsData] {
        
        let query = "SELECT * FROM reports ORDER BY report_date DESC "
       
        var ReportsHolder = [ReportsData]()
        let (resultset, error) = SD.executeQuery(query)
        if (error != nil) {
            SD.printDie(error!)
        } else {
            if resultset.count > 0 {
                for row in resultset {
                    ...
                    let Report = ReportsData()
                    Report.setId((row["_id"]?.asInt()));
                    Report.setMemberId((row["member_id"]?.asString()));
                    Report.setReportTitle((row["report_title"]?.asString()));
                    Report.setReportFileName((row["report_file_name"]?.asString()));
                    Report.setReportDate((row["report_date"]?.asDate()));
                    Report.setReportFormat((row["report_format"]?.asString()));
                    ReportsHolder.append(Report)
                }
    ...
    func delete(forID: Int) {
        let query = "DELETE FROM reports WHERE work_order = " + String(forID)
        let dbn = Database()
        dbn.query(query)
    }
    ...
    func deletePG(forID: String) {
        ...
        let statement = "DELETE FROM reports WHERE work_order = " + String(forID)
        let dbn = Database()
        dbn.query("INSERT INTO " + SyncableQueries.TABLE_NAME
            + " (" + SyncableQueries.SQL_QUERY + ") "
            + " VALUES "
    ...
    class func getLastworkOrder() -> String {
        ...
        let query = "SELECT work_order from reports ORDER BY _id DESC LIMIT 1 "
