...
let AddPillComponent = class AddPillComponent {
    constructor(pillsService, pillHistoryService) {
...
AddPillComponent.ctorParameters = () => [
...
let AppComponent = class AppComponent {
    constructor(pillHistoryService, messageService, http) {
        this.pillHistoryService = pillHistoryService;
        ...
        this.weekNames = [];
        let self = this;
        http.get("http://alexhawley-api.info/pills/history").subscribe((res) => {
        ...
        this.subscription = this.messageService.getMessage().subscribe(message => {
            if (message.doShowForm) {
                this.addPillFormShowing = true;
            }
            else if (message.deletePillId > 0) {
                this.setPillHistoryItem(message.weekNum, message.currentDate, message.deletePillId, 0);
                let body = new _angular_common_http__WEBPACK_IMPORTED_MODULE_4__["HttpParams"]()
                    ...
                    .set('qty', String("0"));
                let ob = this.http.post("http://alexhawley-api.info/pills/delete", body).toPromise()
                    .then(resp => {
                    //console.log("resp: ", resp);
                });
            }
            this.weekNum = message.weekNum;
            this.currentDayInForm = message.currentDate;
        });
    }
    ngOnDestroy() {
        this.subscription.unsubscribe();
    }
    setPillHistoryItem(weekNum, currentDate, pillId, qty) {
        this.pillHistory[weekNum][currentDate].pills[pillId].qty = qty;
    }
    onSubmitAddPill(eventArgs) {
        ...
        let ob = this.http.post("http://alexhawley-api.info/pills/add", body).toPromise()
            .then(resp => {
            //console.log("resp: ", resp);
        });
    }
...
let HistoryDayComponent = class HistoryDayComponent {
...
HistoryDayComponent.ctorParameters = () => [
...
let PillHistoryComponent = class PillHistoryComponent {
    constructor(messageService) {
        this.messageService = messageService;
    ...
    onClickDay($event, weekNum, currentDate) {
        this.messageService.sendMessage({
            weekNum: weekNum,
            doShowForm: true,
...
PillHistoryComponent.ctorParameters = () => [
    { type: src_message_service__WEBPACK_IMPORTED_MODULE_2__["MessageService"] }
];
tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_1__["Input"])(),
...
let PillHistoryService = class PillHistoryService {
    constructor() {
        this.week_names = [
            "Two Weeks Ago",
            "Last Week",
...
let DayNamePipe = class DayNamePipe {
    transform(value) {
        if (!value)
            return null;
        ...
        let finalChar = value.charAt(value.length - 1);
        if (initialChar != "1") {
            switch (finalChar) {
                case "1":
                    return value + "st";
