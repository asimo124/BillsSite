...
let DaysOfWeekComponent = class DaysOfWeekComponent {
    constructor() {
        this.isExpanded = false;
        this.expandHeight = '720';
...
let BillsService = class BillsService {
    ...
    loadBills(curBalance, payDate, hashCode, prevDate, nextDate, testMode = false) {
        ...
        this.http.get('https://budget.hawleywebdesign.com/api/loadBillDates2.php?' + requestParams).subscribe(response => {
    ...
    savePayPeriodNumDays(numDays, payDate) {
        ...
        this.http.get('https://budget.hawleywebdesign.com/api/save_pay_period_num_days.php?' + requestParams).subscribe(response => {
        }, (err) => {
            console.log('error', 'Error loading Growth By Standards : ' + err.error.message);
        });
    }
